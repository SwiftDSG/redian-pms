  ...
  <div class="rd-container">
    <div class="rd-project-menu">
      <div class="rd-project-menu-section">
        ...
        <rd-input-button-small :icon="menu.icon" v-for="menu in projectMenus" :key="menu.name"
          :type="projectMenu === menu.name ? 'primary' : 'default'" @clicked="projectMenuChange(menu.name)" />
      </div>
      <div class="rd-project-menu-section">
        <rd-input-button v-if="projectMenu === 'tasks'" label="Add area" @clicked="openAddArea" />
        <rd-input-button v-else-if="projectMenu === 'reports'" label="Add report" @clicked="openAddReport" />
        <rd-input-select v-else-if="projectMenu === 'users'" :input="roleInput" />
        <rd-input-select v-else :input="areaInput" />
      </div>
    </div>
    <rd-project-overview v-if="projectMenu === 'overview'" :project="project" :state="projectMenuState" :data="{
      projectProgress: projectDataProgress,
      projectTimeline: projectDataTimeline,
    }" @changing-done="projectMenuState = 'idle'" />
    <rd-project-progress v-if="projectMenu === 'progress'" :project="project" :data="projectDataProgress"
      :state="projectMenuState" @changing-done="projectMenuState = 'idle'" />
    <rd-project-timeline v-if="projectMenu === 'timeline'" :project="project" :data="projectDataTimeline"
      :state="projectMenuState" @changing-done="projectMenuState = 'idle'" @edit-task="openPeriodTask" />
    <rd-project-tasks v-if="projectMenu === 'tasks'" :project="project" :data="projectDataAreas" :state="projectMenuState"
      @changing-done="projectMenuState = 'idle'" @add-task="openAddTask" />
    <rd-project-reports v-if="projectMenu === 'reports'" :project="project" :data="projectDataAreas"
      :state="projectMenuState" @changing-done="projectMenuState = 'idle'" @add-task="openAddTask" />
    <rd-project-users v-if="projectMenu === 'users'" :project="project" :data="projectDataTimeline"
      :state="projectMenuState" @changing-done="projectMenuState = 'idle'" @edit-task="openPeriodTask" />
  </div>
</template>
